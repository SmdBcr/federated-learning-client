...
consumer = KafkaConsumer (topicName, group_id = 'client2final',bootstrap_servers = bootstrap_servers,
...
URL_send = 'http://localhost:5000/api/federated/models/send/client2'
    ...
    for message in consumer:
        ...
        if (str(message.value, 'utf-8') == 'start1'):
            ...
            if (checkFirstTrain):
              ...
              second_model.fit(x[countForDataSet],y[countForDataSet],epochs=375,batch_size=10)
        
              countForDataSet +=1
              countForDataSet = min(2,countForDataSet)
              scores=second_model.evaluate(x[countForDataSet],y[countForDataSet])
        
              print("%s:%.2f%%"%(second_model.metrics_names[1],scores[1]*100))
              print("----------------------------------------------------------------------")
        
              second_model.save_weights("second_client_model_weights.h5")
              print("Saved second_client_model_weights to disk")
              print("----------------------------------------------------------------------")
              ...
              json_file.close()
              second_model = model_from_json(second_client)
              second_model.load_weights('updated_weights.h5')
              second_model.compile(loss='binary_crossentropy',optimizer='adam',metrics=['accuracy'])
              second_model.fit(x[countForDataSet],y[countForDataSet],epochs=375,batch_size=10)
              scores=second_model.evaluate(x[countForDataSet],y[countForDataSet])
              print("%s:%.2f%%"%(second_model.metrics_names[1],scores[1]*100))
